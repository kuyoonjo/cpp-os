...
namespace os
{
  ...
  inline std::vector<network_interface_t> filter_network_interfaces(
      const std::vector<network_interface_t> &nis,
      network_interface_filter nif,
      ip_family iff)
  {
    std::vector<os::network_interface_t> filtered;
    std::copy_if(nis.begin(), nis.end(), std::back_inserter(filtered), [&nif, &iff](os::network_interface_t x)
                 {
                   ...
                   if (nif == os::network_interface_filter::has_ipv4)
                     return std::find_if(x.ips.begin(), x.ips.end(), [](os::ip_t ip)
                                         { return ip.family == os::ip_family::ipv4; }) != x.ips.end();
                   if (nif == os::network_interface_filter::has_ipv6)
                     return std::find_if(x.ips.begin(), x.ips.end(), [](os::ip_t ip)
                                         { return ip.family == os::ip_family::ipv6; }) != x.ips.end();
    ...
    if (iff != os::ip_family::all)
      std::transform(filtered.begin(), filtered.end(), filtered.begin(), [&iff](os::network_interface_t x) -> os::network_interface_t
                     {
                       std::vector<os::ip_t> ips;
                       std::copy_if(x.ips.begin(), x.ips.end(), std::back_inserter(ips), [&iff](os::ip_t ip)
                                    { return ip.family == iff; });
                       x.ips = std::move(ips);
                       return x;
                     });
    return filtered;
  ...
  inline std::vector<network_interface_t> network_interfaces()
  {
    ...
    if ((dwRetVal = GetAdaptersAddresses(AF_UNSPEC, 0, NULL, pAddresses, &outBufLen)) == NO_ERROR)
    {
      ...
      while (pAddresses)
      {
        ...
        auto ni = std::find_if(nis.begin(), nis.end(), [&name](network_interface_t x)
                               { return x.name == name; });
        if (ni == nis.end())
        {
          std::array<uint8_t, 6> mac = {pAddresses->PhysicalAddress[0], pAddresses->PhysicalAddress[1], pAddresses->PhysicalAddress[2], pAddresses->PhysicalAddress[3], pAddresses->PhysicalAddress[4], pAddresses->PhysicalAddress[5]};
        ...
        for (i = 0; pUnicast != NULL; i++)
        {
          if (pUnicast->Address.lpSockaddr->sa_family == ip_family::ipv4)
          {
            ...
            auto addr = inet_ntop(ip_family::ipv4, &(sa_in->sin_addr), buff, bufflen);
            ni->ips.push_back({ip_family::ipv4, addr, pUnicast->OnLinkPrefixLength});
          }
          else if (pUnicast->Address.lpSockaddr->sa_family == ip_family::ipv6)
          {
            sockaddr_in6 *sa_in6 = (sockaddr_in6 *)pUnicast->Address.lpSockaddr;
            auto addr = inet_ntop(ip_family::ipv6, &(sa_in6->sin6_addr), buff, bufflen);
            ni->ips.push_back({ip_family::ipv6, addr, pUnicast->OnLinkPrefixLength});
          }
    ...
    if (!rc)
    {
      ...
      {
        auto ni = std::find_if(nis.begin(), nis.end(), [&ifEntry](network_interface_t x)
        ...
        if (ifEntry->ifa_addr->sa_family == ip_family::ipv4)
        {
          ...
          const char *addr = inet_ntop(ifEntry->ifa_addr->sa_family,
                                       ...
                                       buffer,
                                       INET_ADDRSTRLEN);
          auto n = ((struct sockaddr_in *)(ifEntry->ifa_netmask))->sin_addr.s_addr;
          uint8_t cidr = 0;
          while (n > 0)
          {
            if (n & 1)
        ...
        else if (ifEntry->ifa_addr->sa_family == ip_family::ipv6)
        {
          ...
          const char *addr = inet_ntop(ifEntry->ifa_addr->sa_family,
                                       ...
                                       buffer,
                                       INET6_ADDRSTRLEN);
          auto c = ((struct sockaddr_in6 *)(ifEntry->ifa_netmask))->sin6_addr.s6_addr;
          int i = 0;
          uint8_t cidr = 0;
          unsigned char n = 0;
          while (i < 16)
        ...
        {
          auto ptr = (unsigned char *)LLADDR((struct sockaddr_dl *)ifEntry->ifa_addr);
          for (int i = 0; i < 6; ++i)
            ni->mac[i] = ptr[i];
        }
#endif
